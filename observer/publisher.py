...
def host_observations():
    ...
    result = [observation("host", signal, "host_metrics", data[key], unit=unit)
              for key, (signal, unit) in mapping.items()]
    result.extend([
        observation("host", "disk.root.used_percent", "host_metrics",
                    round(data["root_used_bytes"] / data["root_total_bytes"] * 100, 2),
                    unit="percent"),
        observation("host", "disk.root.inodes_used_percent", "host_metrics",
                    data["root_inode_used_percent"], unit="percent"),
        observation("host", "disk.backup.used_percent", "host_metrics",
                    round(data["backup_used_bytes"] / data["backup_total_bytes"] * 100, 2),
                    unit="percent"),
        observation("host", "disk.backup.inodes_used_percent", "host_metrics",
