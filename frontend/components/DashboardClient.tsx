...
type Environment={id:string;name:string}; type Snapshot={overall_state:string;observed_at:string|null;observations:Observation[]}; type Incident={id:string;title:string;severity:string;lifecycle:string;last_seen_at:string};
...
export function DashboardClient() {
  ...
  return <AppShell><main className="dashboard"><div className="title-row"><div>
    ...
    <div className={`health-banner state-${(snapshot?.overall_state??"UNKNOWN").toLowerCase()}`}>Overall: {snapshot?.overall_state??"UNKNOWN"}<small>{snapshot?.observed_at?` Laatste meting ${new Date(snapshot.observed_at).toLocaleString("nl-NL")}`:" Nog geen metingen"}</small></div><StatusGrid observations={snapshot?.observations??[]} />
    <section className="lower-grid"><article className="panel"><h2>Actieve incidenten</h2>{incidents.filter(i=>i.lifecycle!=="RESOLVED").length?incidents.filter(i=>i.lifecycle!=="RESOLVED").map(i=><div className="incident" key={i.id}><b>{i.severity}</b> {i.title}<small>{new Date(i.last_seen_at).toLocaleString("nl-NL")}</small></div>):<p>Geen actieve incidenten</p>}</article><article className="panel"><h2>Monitoring</h2><p>Alleen-lezen signalen voor Web, Backups en Host.</p></article></section>
  </main></AppShell>;
}
